...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        while ((de = readdir(dr)) != NULL){
            ...
            char fileProcessing[256];
            
            if(de->d_name){
                
                strcat(fileProcessing, currdir);
                strcat(fileProcessing, de->d_name);
                strncpy(input, fileProcessing, 256);
                printf("%s %s\n", fileProcessing, input);
            } else {
            ...
            int i, j;
            //for(i = 0; i<nboxes; ++i){
            //    int class = dets[i].sort_class;
                //if(dets[i].prob > thresh){
            //        printf("%f, %f, %f, %f, %lf, %lf, %lf, %d, \n", dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h, dets[i].prob, dets[i].mask, dets[i].objectness, class);
                //}
            //}
            char fileName[1024];
            ...
            for(i = 0; i < nboxes; ++i){
                ...
                for(j = 0; j < l.classes; ++j){
                    if (dets[i].prob[j] > thresh){
                        ...
                        //printf("%s", fileName);
                        fprintf(fp, "%f, %f, %f, %f, %s  \n", (dets[i].bbox.x - dets[i].bbox.w/2) * im.w, (dets[i].bbox.y-dets[i].bbox.h/2) *im.h, dets[i].bbox.w*im.w, dets[i].bbox.h*im.h, labelstr);
            ...
            free_image(im);
            free_image(sized);
            memset(fileProcessing, 0, sizeof fileProcessing);
